...
// src/main.ts
var import_elma = __toESM(require("elma.node"));
...
async function main() {
  ...
  let levs = [];
  if (args.length == 0) {
    levs = new Array(54).fill(0).map((_2, i) => i);
  } else {
    levs = args.flatMap((a) => {
      ...
      return m.map((l) => import_path.default.basename(l));
    });
  }
  import_elma.default.elmaInit();
  import_elma.default.setQuality(1);
  import_elma.default.setDT(0.182 * 24e-4 * (1e3 / FPS));
  for (let l = 0; ; ) {
...
async function gameloop(lev) {
  console.log("lev", lev);
  let frame = 0;
  let slots = new Slots(lev, import_elma.default.initGame(lev));
  let paused = false;
  let frameEdit = 0;
  import_elma.default.setGLRenderCallback(() => renderTAS(slots, paused, frame, frameEdit));
  function render() {
    frame = slots.slot.frame;
    import_elma.default.render(slots.slot.sim);
  }
  let _speed = 1;
  let lastRender = timing();
  import_elma.default.handleEvents();
  for (let l = 0; ; l++) {
    let speed = _speed;
    let doRender = l % 30 == 0;
    let done = slots.sim.dead() || slots.sim.finished();
    const lctrl = import_elma.default.keyIsDown(SSC.LCTRL);
    const lshift = import_elma.default.keyIsDown(SSC.LSHIFT);
    if (lctrl && import_elma.default.keyJustPressed(SSC.N)) {
      return "next";
    } else if (lctrl && import_elma.default.keyJustPressed(SSC.P)) {
      return "prev";
    }
    if (import_elma.default.keyJustPressed(SSC.S)) {
      slots.save();
    } else if (import_elma.default.keyJustPressed(SSC.L)) {
      paused = true;
      slots.load();
      render();
    }
    if (!lctrl) {
      if (import_elma.default.keyJustPressed(SSC.UP) || import_elma.default.keyJustPressed(SSC.DOWN)) {
        slots.slot.truncate();
        paused = false;
        frameEdit = 0;
      }
    }
    if (lctrl && import_elma.default.keyJustPressed(SSC.Q)) process2.exit();
    if (lshift && import_elma.default.keyJustPressed(SSC.ESCAPE)) process2.exit();
    if (!lctrl && import_elma.default.keyJustPressed(SSC.RETURN)) {
      paused = !paused;
      frameEdit = 0;
    }
    if (paused && import_elma.default.keyJustPressed(SSC.N)) {
      slots.slot.step();
      slots.save();
      render();
    }
    if (!lctrl && paused && import_elma.default.keyJustPressed(SSC.B)) {
      slots.slot.back();
      slots.save();
      render();
    }
    if (!lctrl && import_elma.default.keyJustPressed(SSC.R)) {
      paused = true;
      slots.slot.reset();
      render();
    }
    if (!lctrl && import_elma.default.keyJustPressed(SSC.BACKSPACE)) slots.slot.truncate();
    if (import_elma.default.keyJustPressed(SSC.APOSTROPHE)) {
      speed = _speed = lshift ? 0.6 : 0.3;
    } else if (import_elma.default.keyJustPressed(SSC.SEMICOLON)) {
      speed = _speed = lshift ? 0.075 : 0.15;
    } else if (import_elma.default.keyJustPressed(SSC.SLASH)) {
      speed = _speed = 1;
    } else {
      let lbrac = import_elma.default.keyIsDown(SSC.LEFTBRACKET);
      let rbrac = import_elma.default.keyIsDown(SSC.RIGHTBRACKET);
      if (lbrac || rbrac && !done) {
        paused = true;
        let n7 = lctrl ? 1 : lshift ? 12 : 4;
        lbrac ? slots.slot.back(n7) : slots.slot.step(n7);
    ...
    if (paused) {
      ...
      if (events.length) {
        let [f, k2] = events[frameEdit];
        if (import_elma.default.keyIsDown(SSC.LEFT)) {
          if (!lctrl || import_elma.default.keyJustPressed(SSC.LEFT)) {
            if (f > 0) {
              events.push([f - 1, k2]);
        ...
        } else if (import_elma.default.keyIsDown(SSC.RIGHT)) {
          if (!lctrl || import_elma.default.keyJustPressed(SSC.RIGHT)) {
            events[frameEdit][0]++;
            if (frameEdit < 7) {
        ...
        } else if (lctrl && import_elma.default.keyJustPressed(SSC.UP)) {
          frameEdit = Math.max(0, frameEdit - 1);
          doRender = true;
        } else if (lctrl && import_elma.default.keyJustPressed(SSC.DOWN)) {
          frameEdit = Math.min(7, frameEdit + 1);
          doRender = true;
        }
      }
    }
    if (!lctrl && import_elma.default.keyJustPressed(SSC.Q)) {
      import_elma.default.setQuality(101);
    }
    if (import_elma.default.keyIsDown(SSC.MINUS)) {
      import_elma.default.adjustZoom(-0.01);
      doRender = true;
    }
    if (import_elma.default.keyIsDown(SSC.EQUALS)) {
      import_elma.default.adjustZoom(0.01);
      doRender = true;
    }
    if (!paused && !done) {
    ...
    let ms2 = 1e3 / FPS / speed;
    let now = timing();
    let delay = lastRender + ms2 - now;
    if (delay > 0) import_elma.default.sleep(delay);
    doRender && render();
    lastRender = delay > 0 ? lastRender + ms2 : now;
    import_elma.default.handleEvents();
  }
}
var Slots = class {
...
var CHECKPOINT_INTERVAL = 30;
var Slot = class _Slot {
  constructor(sim, shadow = false) {
    this.sim = sim;
    this.shadow = shadow;
    this.lastspeed = 0;
    this.sim = sim;
  ...
  copy() {
    let slot = new _Slot(this.sim.copy());
  ...
  step(n7 = 1) {
    for (let i = 0; i < n7; i++) {
      ...
      if (this.frame < this.keys.length) {
        ...
        let alo = import_elma.default.keyIsDown(SSC.RCTRL);
        keys = (import_elma.default.keyIsDown(SSC.UP) ? 1 : 0) + (import_elma.default.keyIsDown(SSC.DOWN) ? 2 : 0) + (import_elma.default.keyIsDown(SSC.LEFT) || alo ? 4 : 0) + (import_elma.default.keyIsDown(SSC.RIGHT) || alo ? 8 : 0) + (import_elma.default.keyJustPressed(KEY_TURN) ? 16 : 0);
      ...
      import_elma.default.step(this.sim, keys, []);
      this.frame++;
      this._updateSpeed();
    }
  }
  back(n7 = 1) {
    for (let j2 = 0; j2 < n7; j2++) {
  ...
  _updateSpeed() {
    let { x: x2, y } = this.sim.body_r();
    if (this.lastpos) {
      let hyp = Math.sqrt((x2 - this.lastpos.x) ** 2 + (y - this.lastpos.y) ** 2);
  ...
  getKeyEvents(n7 = 5) {
    ...
    return events.reverse().slice(0, n7);
  }
  patch(events) {
    let diff = this.frame - events[0][0];
    for (let [f2, k2] of events) {
      this.keys[f2] = k2;
    }
    this.sim = this.checkpoints[0].copy();
    this.checkpoints.length = 1;
  ...
  write(lev) {
    if (!import_fs2.default.existsSync("libelma_saves")) {
      import_fs2.default.mkdirSync("libelma_saves");
    }
    ...
    import_fs2.default.writeFileSync(`libelma_saves/${lev}.save.json`, data);
  }
  load(lev) {
    let f = `libelma_saves/${lev}.save.json`;
    if (import_fs2.default.existsSync(f)) {
      let data = JSON.parse(import_fs2.default.readFileSync(f, "utf8"));
      this.keys = data.keys;
...
function renderTAS(slots, paused, frame, frameEdit) {
  ...
  let ms2 = String(Math.floor(f / FPS * 100 % 100)).padStart(2, "0");
  let time = `${mins}:${secs}:${ms2}`;
  let head = `TIME        FRAME   SPEED   KEYS   STATUS`;
  let slot0 = `${time}   ${String(f).padStart(6, " ")}  ${(slots.slot.lastspeed * 10).toFixed(2).padStart(6)}   ${slots.slot.keysRendered}  `;
  let a = 0.2;
  import_elma.default.renderBox(0, 0, 0, a, 10, 10, 2, head.length + 2, 15, 18);
  import_elma.default.renderText(1, 1, 1, 0.6, 25, 25, 0, 0, head);
  import_elma.default.renderText(1, 1, 1, 0.8, 25, 29, 0, 1, slot0);
  let status = slots.slot.status;
  if (status == "DEAD") {
    import_elma.default.renderText(1, 0.2, 0.2, 0.8, 25, 29, slot0.length, 1, status);
  } else {
    import_elma.default.renderText(1, 1, 1, 0.8, 25, 29, slot0.length, 1, status);
  }
  if (paused) {
    let events = slots.slot.getKeyEvents(8);
    import_elma.default.renderBox(0, 0, 0, a, -10, 80, 10, 12, 15, 15);
    import_elma.default.renderText(1, 1, 1, 1, -25, 95, 0, 0, "PAUSED      ");
    if (events.length) {
      import_elma.default.renderBox(0, 1, 1, a, -10, 130 + frameEdit * 20, 1, 12, 15, 6);
    }
    events.forEach(([f2, k2], i) => {
      import_elma.default.renderText(1, 1, 1, 1, -25, 95, 0, 2 + i, `${String(f2).padStart(6, " ")}  ${renderKeys(k2)}`);
    });
  }
}
function renderKeys(k2) {
  return `${k2 & 1 ? "G" : "_"}${k2 & 2 ? "B" : "_"}${k2 & 4 ? "L" : "_"}${k2 & 8 ? "R" : "_"}${k2 & 16 ? "T" : "_"}`;
}
function timing() {
  let [sec, ns2] = process2.hrtime();
  return sec * 1e3 + ns2 / 10 ** 6;
}
var SSC = {
  RETURN: 40,
  SEMICOLON: 51,
...
var KEY_TURN = SSC[process2.env["ELMA_KEY_TURN"] || "SPACE"];
main();
